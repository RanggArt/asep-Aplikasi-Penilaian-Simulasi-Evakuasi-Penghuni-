...
<title>Formulir Penilaian Koordinator Lapangan / FSM</title>
...
<div class="top">
  <div class="top-inner wrap" style="padding:0;">
    ...
    <h1>Formulir Penilaian Koordinator Lapangan / FSM</h1>
    <p>Penilaian kinerja Koordinator Lapangan / Fire Safety Manager (FSM) dalam memimpin dan mengoordinasikan simulasi evakuasi darurat kebakaran.</p>
  </div>
</div>
<a href="{{ route('home') }}" style="color:white; font-size:14px; text-decoration:underline;">&larr; Kembali ke Menu Utama</a>
<div class="wrap">
  ...
  <div class="section-head">
    <h2>Penilaian Koordinator Lapangan / FSM</h2>
    <span id="filledCount">0/4 diisi</span>
  </div>
  ...
  <div class="summary card-shell">
    ...
    <div class="total-row">
      <div class="total-left">
        <div class="n"><span id="totalNilai">0</span> / 16</div>
  ...
  <footer>Formulir Penilaian Koordinator Lapangan / FSM &mdash; Sudin Penanggulangan Kebakaran dan Penyelamatan Kota Administrasi Jakarta Utara</footer>
</div>

<div class="progress-bar">
  <span class="progress-label" id="progressLabel">0%</span>
  <div class="progress-track"><div class="progress-fill" id="progressFill"></div></div>
  <span class="progress-label" id="progressScore">0/16</span>
</div>

<script>
(function(){
  const INDICATORS = [
    { title: "Pengambilan Keputusan Cepat", desc: "Merespons situasi darurat dengan segera dan tepat." },
    { title: "Efektivitas Komunikasi", desc: "Memberikan instruksi yang jelas dan terkoordinasi kepada tim ERT." },
    { title: "Kontrol dan Pengawasan", desc: "Mengawasi jalannya evakuasi secara keseluruhan." },
    { title: "Kerjasama dengan Pihak Eksternal (Opsional)", desc: "Jika ada simulasi koordinasi dengan pemadam kebakaran/polisi/aparat pemerintah lainnya.", optional: true }
  ];
  const SCALE = [
  ...
  function render(){
    ...
    els.filled.textContent = `${answered}/4 diisi`;
    ...
    const pct = Math.round((answered/4)*100);
    els.pFill.style.width = pct + '%';
    els.pLabel.textContent = pct + '%';
    els.pScore.textContent = total + '/16';
  ...
  document.getElementById('btnPdf').addEventListener('click', function(){
    ...
    const emptyIdx = state.scores.findIndex((v, i) => v === 0 && !INDICATORS[i].optional);
    if (emptyIdx !== -1){
      ...
      alert(`Poin nomor ${emptyIdx + 1} ("${INDICATORS[emptyIdx].title}") belum dinilai. Indikator 1-3 wajib diisi; poin 4 (Kerjasama dengan Pihak Eksternal) bersifat opsional (kolom Keterangan boleh dikosongkan).`);
      return;
    }
    ...
    try{
      ...
      doc.text('Penilaian Koordinator Lapangan / FSM', pageW/2, y, { align:'center' });
      y += 3;
      ...
      doc.setFontSize(8.5);
      doc.setTextColor(34,28,26);
      doc.text(`Total Nilai: ${lastTotal} / 16   |   Rata-rata: ${lastRata.toFixed(1)}   |   Predikat: ${lastPredikatText}`, marginX, afterTableY);
      ...
      const fileParts = ['hasil-penilaian-koordinator-fsm'];
